...
def _evaluate_config(args: tuple) -> dict | None:
    ...
    # sanity básico: slow > fast
    if slow_ema <= fast_ema:
        return None

    strat_cfg = MACDADXTrendStrategyConfig(
        fast_ema=fast_ema,
        slow_ema=slow_ema,
    ...
    strategy = MACDADXTrendStrategy(config=strat_cfg)
...
def _build_param_grid(
    fast_emas: Iterable[int],
    slow_emas: Iterable[int],
    signal_emas: Iterable[int],
    trend_ema_windows: Iterable[int],
    adx_windows: Iterable[int],
...
def main():
    ...
    if not rows:
        print("MACD+ADX: sin resultados (revisa min_trades o rangos).")
        return
